...
pub fn get_safe_path(root_dir: &str, relative_path: &str) -> Result<PathBuf> {
    ...
    let canonical_root = root_dir_path.canonicalize().map_err(|_| {
        Error::Tool("Failed to canonicalize the root dir".to_string())
    })?;

    let canonical_path = path.canonicalize().map_err(|_| {
        Error::Tool(format!("Failed to canonicalize path: {}", path.display()))
    })?;
